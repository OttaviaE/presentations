...
\begin{frame}
	...
	\begin{exampleblock}{Respondents are random}
		
		Sampled from a lager population
...
\begin{alertblock}{Stimuli/item are fixed}
...
\begin{frame}{However...}
	...
	\begin{example}{{\color{rasch}{Positive}} vs. \alert{Negative} attributes in Linguistic}
		
		There is a universe of positive words 
		
		Only a sample of positive attributes (e.g., good, nice) and negative attributes (e.g., bad, evil) are administered 
...
\begin{frame}{}
	...
	\small
	The results can be generalized at the respondents level but not at the stimulus level $\rightarrow$ Results can be generalized if and only if the exact same set of stimuli is used 
	...
	\begin{block}{Robustness of the results}
		...
		Averaging across stimuli to obtain a person-level score results in biased estimates due to the noise in the data
	\end{block}
	
	\begin{block}{Loss of information}
		
		\small
		Every stimulus is assumed to be equal $\rightarrow$ to have the same effect on the observed score
