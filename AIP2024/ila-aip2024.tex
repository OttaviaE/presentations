...
%	\end{columns}
	\footnotesize
		...
		Item bank $B$, $||B|| = N$, items $i = \{1,2, \ldots, N\}$
...
%			
			...
			\includegraphics[width=.60\textwidth]{img/TIF-first.pdf}
		     ...
		     $Q^1 = Q^{0} \cup \arg\min_{i \in B\setminus Q^0} |\theta_{target} - b_i|$, 	$||Q^1|| = 1$ \\
			...
			$Q^3 = Q^{2} \cup \arg\min_{i \in B\setminus Q^2} |\theta_{target} - b_i|$, 	$||Q^3|| = 3$ \\
			 $|\mathbf{TIF}^* - \mathbf{TIF}^3| \geq |\mathbf{TIF}^* - \mathbf{TIF}^{2}|$ $\rightarrow$ \texttt{false} $\rightarrow$ \textbf{end}, $Q_{\textbf{ILA}} = Q^2$
		\end{overprint}
	\end{frame}
...
\begin{frame}{Brute Force Procedure -- BFP}
	...
	\footnotesize
	Item bank $B$, $||B|| = N$\\
	Possible $Q_m \subset B$ item combinations of different lengths $l = 1, 2, \ldots, N-1$\\
