---
title: "There's nothing more practical than a good theory"
subtitle: "From theoretical conceptualization to practical implementation of item response theory algorithms for item selection"
author: "**Ottavia M. Epifania**$^{1, 2}$"
institute: "$^1$ Psicostat, $^2$ University of Trento, Rovereto"
date: today
format: 
  revealjs: 
    theme: mytheme.scss
...
library(magick)
setwd("psicostatMeeting/img")
files = dir()
names = gsub(".pdf", ".png", files)
for (i in 1:length(files)) {
  img <- image_read_pdf(files[i], density = 300)
image_write(img, names[i])
}
...
**Max**mize the **min**imun measurement precision in specific regions of interest for the assessment provided by test $Q$
...
**Min**imize the **max**imum distance from a target function that describes the desired measurement precision of test $Q$ 
...
ylab(expression(paste("P(", x[p][i],  "= 1|", theta[p], ", ", b[i], ", ", a[i], ", ", c[i], ", ", d[i],  ")"))) + 
 ...
 annotate(
  "text",
  x = -1.5, y = 0.15,
  label = as.character(expression(paste(b[1], " = ", 0))),
  parse = TRUE,
  hjust = 0,
  size = 14,
  color = myitem[1]
  ...
  "text",
  x = -0.7, y = 0.50,
  label = as.character(expression(paste(a[1], " = ", 1))),
  parse = TRUE,
  hjust = 0,
  size = 14,
  color = myitem[1]
  ...
  "text",
  x = 1.5, y = 0.15,
  label = as.character(expression(paste(b[2], " = ", 1))),
  parse = TRUE,
  hjust = 0,
  size = 14,
  color = myitem[2]
  ...
  "text",
  x = 1.5, y = 0.50,
  label = as.character(expression(paste(a[2], " = ", 1.7))),
  parse = TRUE,
  hjust = 0,
  size = 14,
  color = myitem[2]
...
ylab(expression(paste("P(", x[p][i],  "= 1|", theta[p], ", ", b[i], ", ", a[i], ", ", c[i], ", ", d[i],  ")"))) + 
 ...
 annotate(
  "text",
  x = -1.5, y = 0.15,
  label = as.character(expression(paste(c[3], " = ", 0.10))),
  parse = TRUE,
  hjust = 0,
  size = 14,
  color = myasy[1]
  ...
  "text",
  x = .7, y = 0.90,
  label = as.character(expression(paste(e[3], " = ", 0.98))),
  parse = TRUE,
  hjust = 0,
  size = 14,
  color = myasy[1]
  ...
  "text",
  x = -2.5, y = 0.35,
  label = as.character(expression(paste(c[4], " = ", 0.20))),
  parse = TRUE,
  hjust = 0,
  size = 14,
  color = myasy[2]
  ...
  "text",
  x = 1.5, y = 0.80,
  label = as.character(expression(paste(e[4], " = ", 0.95))),
  parse = TRUE,
  hjust = 0,
  size = 14,
  color = myasy[2]
...
rownames(allitems) = paste("Item", nrow(allitems):1)
...
test = theta_target(targets, item_par)
...
kable(dh, 
  ...
  # Evidenzia le righe (come fai già)
...
item_par = data.frame(b = runif(n, -3,3),          # <1>
                      ...
                      e = 1)                       # <1>
theta = rnorm(1000) # <2> 
test = bench(item_par, theta = theta, num_item = 3)# <3>
```
...
You can find the slides on my personal page [https://ottaviae.github.io/presentations](https://ottaviae.github.io/presentations)

## What is Psicostat?

<br>

![](img/chi_siamo_mod.png){fig-align="center" height="500px"}
...
- **Stay tuned!** [https://psicostat.dpss.psy.unipd.it/pages/meetings.html](https://psicostat.dpss.psy.unipd.it/pages/meetings.html)
:::
...
::: {layout-nrow=3}
![Gianmarco Altoè](img/altoe.png){height="100px"}

![Massimiliano Pastore](img/pastore.png){ height="100px"}

![Livio Finos](img/finos.png){fig-align="center" height="100px"}

![Giulia Calignano](img/calignano.png){fig-align="center" height="100px"}

![Ottavia Epifania](img/epifania.png){fig-align="center" height="100px"}

![Filippo Gambarota](img/gambarota.jpg){fig-align="center" height="100px"}

![Enrico Toffalini](img/toffalini.png){fig-align="center" height="100px"}

![Luca Menghini](img/menghini.png){fig-align="center" height="100px"}

![Ambra Perugini](img/perugini.png){fig-align="center" height="100px"}

![Margherita Calderan](img/calderan.png){fig-align="center" height="100px"}

![Marco Tullio Liuzza](img/liuzza.png){fig-align="center" height="100px"}

![Tommaso Feraco](img/feraco.png){fig-align="center" height="100px"}
:::
