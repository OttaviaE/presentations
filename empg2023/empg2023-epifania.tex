...
\setbeamertemplate{frametitle}{\color{template}\bfseries\scshape\insertframetitle\par\vskip2pt\hrulefill}
...
\titlegraphic{%
	\includegraphics[width=1.8cm,height=1.8cm,keepaspectratio]{img/unipd.png}
}
\begin{document}
...
\begin{frame}
	...
	\end{textblock*}
		...
		\begin{textblock*}{2cm}(1.5cm,7.5cm)
	...
	\begin{alertblock}{Issue}
		...
		\begin{itemize}
			\item Can only be administered once (o after a long time period)
	...
	\begin{exampleblock}{Aim}
		...
		Development of an open-source easy-to-use tool for the automatic (and reproducible) generation of Raven-like matrices
	\end{exampleblock}
...
\begin{tikzpicture}
	...
	\highlightbox[difference,very thick]{0.26}{0.52}{0.48}{.86}{circle,myGreen}{}{4-}
	\highlightbox[difference,very thick]{0.01}{0.02}{0.21}{.4}{circle,myGreen}{}{4-}
	...
	\highlightbox[ic,very thick]{0.01}{0.52}{0.21}{.86}{circle,myGreen}{}{6-}
\end{tikzpicture}
...
\vspace{5mm}
	...
	\scalebox{.95}{
		...
		\textcolor<3->{repetition}{Repetition} &  Repetition of a cell \textbf{adjacent} to the blank space \\ 
		\textcolor<4->{difference}{Difference} &  Different in appearance from every element of the matrix\\
		\textcolor<5->{wp}{Wrong Principle} &  Copy of a cell or combination of cells \\
		\textcolor<6->{ic}{Incomplete Correlate} &  \textbf{Almost} the correct response \\
...
\begin{frame}
	...
	\begin{tabular}{p{2cm}p{4cm} p{10cm}}
		\hline
		Rules Category	&	Rule name	&	Definition	\\
		\hline
		Visuospatial	&	Object addition/subtraction	&	Visually merge two elements 	\\
		...
		\hline
		Logical	&	AND	&	The third cell contains ONLY the elements that appeared in both the first and second cells 	\\
...
\end{center}
	...
	\vspace{3mm}
	\begin{quote}
		Objects \hspace{2mm} Rules \hspace{2mm} Matrix generator \hspace{2mm} Response options generator
	\end{quote}
	\end{itemize}
...
\begin{frame}{The mat\texttt{R}iks  architecture: Matriks generator}
		...
		\begin{tikzpicture}[node distance=1.5cm]
			...
			\node [xshift =+0.3cm, text width=4cm,font=\footnotesize, below of = start]
			{
	...
	\begin{figure}
		...
		\onslide<3->
			...
			\scalebox{.70}{
				...
				Incomplete Correlate & Repetition & Wrong Principle & Difference\\
			\end{tabular}
			}
		};
			\onslide<2-> \path [->,shorten >= +3pt] (matrix.south) edge (start.north);
			\onslide<3-> \path [->, shorten >= +2pt] (start.south) edge (response.north);
		\end{tikzpicture}
	\end{figure}
\end{frame}
