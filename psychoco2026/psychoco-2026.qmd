...
ggplot(myiifs, 
       aes(x = theta, y = value, color = name, group = name)) + geom_line(linewidth = 1.2) + theme_bw() +    scale_color_manual(values = c(myitem, myasy))+ geom_line(linewidth=1.2) + 
  xlab(expression(theta)) + 
...
kable(dh, format = "html", escape = F, align = "ccc", caption = "IIF Matrix \\(k = 0\\)") %>%
...
kable(disph, format = "html", escape = F, align = "ccc", caption = "IIF Matrix \\(k = 1\\)") %>%
  kable_styling(full_width = F, font_size = 20) %>%
  column_spec(1:3, border_left = TRUE, border_right = TRUE) %>%
  column_spec(oldmax[2], background = "white") %>% 
  row_spec(highlight_cells, extra_css = "background-color: #F0B97A;") %>% 
  row_spec(1, extra_css = "border-top: 1px solid;") %>% 
    row_spec(10, extra_css = "border-bottom: 1px solid;")
disph = start
...
highlight_cells = highlight_cells[-oldmax[1]]
kable(disph, format = "html", escape = F, align = "ccc", caption = "IIF Matrix \\(k = 2\\)") %>%
  kable_styling(full_width = F, font_size = 20) %>%
  column_spec(1:3, border_left = TRUE, border_right = TRUE) %>%
  column_spec(c(2,oldmax[2]), background = "white") %>% 
  row_spec(highlight_cells, extra_css = "background-color: #F0B97A;") %>% 
  row_spec(1, extra_css = "border-top: 1px solid;") %>% 
    row_spec(10, extra_css = "border-bottom: 1px solid;")
disph = start
...
kable(disph, format = "html", escape = F, align = "ccc", caption = "End") %>%
  kable_styling(full_width = F, font_size = 20) %>%
  column_spec(1:3, border_left = TRUE, border_right = TRUE) %>%
...
5. Plot the resulting TIF (as compared to the TIF obatined from $B$)
...
The benchmark procedure highly relies on the quality of the items on the item, does not allow for any control on the latent trait levels for which the information needs to be maximized

Still affected by the quality of the items in the item bank, but the $\theta$-target procedure "forces" the item selection to consdier the entire latent trait or to focus on specific levels
